...
\subsubection{Implementation}

If we model the concentration along 1 dimension as a simple function we can then sample a concentration at different distances. We have an example of sampling a function that shows concentration distributed as the standard-normal distribution:
\begin{center}
    \includegraphics[width=12cm]{images/show_sampling.png}
\end{center}

Now we have these points, using central difference equations, we can find approximations for our $\frac{\partial C}{\partial x}$ and our $\frac{\partial^2 C}{(\partial X)^2}$. If we number the nodes $C_1, C_2, C_3...$ then we get that:

\begin{equation}
    \frac{\partial C_i}{\partial x} \approx \frac{F(C_{i+1}) - F(C_{i-1})}{2h}
\end{equation}
\begin{equation}
    \frac{\partial C_i}{\partial x} \approx \frac{F(C_{i+1}) -2F(C_i) + F(C_{i-1})}{h^2}
\end{equation}

Where $h$ is the distance between each sample point. If we want to generalize this for all points we can use the matrices:
