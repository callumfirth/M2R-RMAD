...
Consider the simplified 1 dimensional advection-diffusion equation given by Equation \ref{AdvectionDiffusion} where we are measuring the concentration of some liquid over time, also accounting for the velocity of the liquid, $V$, and the rate of diffusion, $D$.
...
\begin{equation}
    C_{new} = C_{old} + \Delta t \frac{\partial C_{new}}{\partial t}
    \label{newC}
\end{equation}

We can approximate $\frac{\partial C}{\partial t}$ in Equation \ref{AdvectionDiffusion} by using $A, B$ denoted in Equation \ref{CentralDifference} and, then further plug this approximation into Equation \ref{newC} to get.
\begin{equation}
    \begin{split}
        & \frac{\partial C}{\partial t} \approx -VA + DB \\
        \Longrightarrow \ & C_{new} = C_{old} + \Delta t (-VA + DB) C_{new} \\
        \Longrightarrow \ & [I + \Delta t(VA - DB)]C_{new} = C_{old}
    \end{split}
\end{equation}
Then defining $M$ as
\begin{equation}
    \begin{split}
        & M := I + \Delta t(VA - DB) \\
        \Longrightarrow \ &  C_{new} = M^{-1} C_{old}
    \end{split}
\end{equation}
...
\begin{figure}[h]
    ...
    \caption{Plot of Equation \ref{AdvectionDiffusion} with V=10, D=5, plotting from t=0 to t=1.5 at timesteps of 0.1. Dotted line is value at t=0}
    \label{fig:ODE}
\end{figure}
